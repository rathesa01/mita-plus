...
⚠️ ANTI-TEMPLATE RULES — ห้ามทำ:
1. ใช้ pattern หรือ phrasing เดียวกับ creator คนอื่น — ทุก output ต้อง unique ตาม contentDescription
2. ขึ้นต้น shock ด้วย "creator นี้มี view X แต่ทุก 1000 view..." — ห้ามใช้โครงสร้างนี้
3. Paraphrase blueprint แล้วเปลี่ยนแค่ตัวเลข — ต้อง reason จาก contentDescription จริงๆ
...
export function buildUserPrompt(
  ...
  // ── Monetization posture (what they already have) ────────────────
  const monetizationPosture = [
    data.hasAffiliate && '✅ Affiliate (active)',
    data.hasProduct && '✅ สินค้า/บริการตัวเอง',
    data.hasFunnel && '✅ Funnel/ระบบดักลูกค้า',
...
รายได้จากที่ไหนตอนนี้: ${currentSourcesText} ← ⚠️ ห้ามแนะนำวิธีเหล่านี้ซ้ำ
...
ช่องทางทำเงินที่มีอยู่แล้ว: ${monetizationPosture}
รายได้ปัจจุบัน: ${currentIncomeDisplay}

⚠️ กฎสำคัญ:
- ถ้า creator มีช่องทางใดอยู่แล้ว → ห้ามแนะนำว่า "ขาด" ช่องทางนั้น
- ให้แนะนำว่า "optimize" ช่องทางที่มี หรือเสริมช่องทางใหม่ที่ complementary
- ตัวอย่าง: ถ้ามี affiliate → อย่าบอก "สมัคร affiliate" ให้บอกว่า "เปลี่ยน partner / ขึ้น tier / เพิ่ม category"

=== PLATFORM ===
${platformCtx}

=== NICHE BLUEPRINT: ${nicheTH} (reference เสริม — ไม่ใช่ primary) ===
...

{
  ...
  "whyItHappens": "${
    data.hasAffiliate && data.monthlyIncome > 0
      ? `Creator นี้มี affiliate active + รายได้ ${currentIncomeDisplay} แล้ว · diagnose ทำไมไม่ scale — ห้ามบอกให้ 'ทำ affiliate' (มีแล้ว) ให้ระบุ: เปลี่ยน partner / ขึ้น tier / เปลี่ยน CTR funnel / เพิ่ม category สินค้า · อ้างถึง content จริง: '${contentDescription || subNiche}' · จบด้วย 'จุดเปลี่ยน: [approach ที่ต่างออกไปพร้อมตัวอย่าง]'`
      : data.hasAffiliate
        ? `มี affiliate setup แต่ยังไม่มีรายได้ · diagnose CTR / สินค้าผิด / trust gap · ห้ามบอก 'สมัคร affiliate' (มีแล้ว) · อ้าง content: '${contentDescription || subNiche}' และ biggestProblem: '${data.biggestProblem}' · จบด้วย 'จุดเปลี่ยน: [วิธีที่ต่างออกไป]'`
        : hasTriedSomething
          ? `COUNTER-INTUITIVE ROOT CAUSE — อธิบายว่าทำไม ${triedText} ถึงไม่ work สำหรับ content '${contentDescription || subNiche}' บน ${platformTH} โดยเฉพาะ · ต้องเป็นเหตุผลที่คนทั่วไปไม่รู้ · เจาะที่พฤติกรรม audience ${audienceTH} ว่าทำไมถึงไม่ซื้อ · จบด้วย 'จุดเปลี่ยน: [approach ใหม่ที่ต่างออกไป 180 องศา]'`
          : `COUNTER-INTUITIVE ROOT CAUSE — อธิบายว่าทำไม ${data.name} ที่ทำ content '${contentDescription || nicheTH}' มานาน ${durationTH} บน ${platformTH} ถึงยังไม่มีรายได้ · ต้องเป็น insight ที่คนทั่วไปไม่รู้ · เกี่ยวกับพฤติกรรม audience ${audienceTH} โดยเฉพาะ · จบด้วย 'จุดเปลี่ยน: [มุมมองที่ต้องเปลี่ยน 180 องศาพร้อมตัวอย่าง]'`
  }",

  "topActions": "สรุป 1 ประโยคสั้นๆ ว่าแผนนี้คืออะไร เช่น 'เริ่มจาก TikTok Shop Affiliate ก่อน ใช้เวลา 3 วัน ไม่มีต้นทุน'",

  "actionSteps": [
    {
      "day": "วันที่ 1",
      "time": "XX นาที",
      "title": "ชื่อ action สั้นๆ ระบุชื่อ platform/program จริง",
      "detail": "ขั้นตอนที่ทำได้เลย: URL จริง → ทำอะไร → ได้อะไร ห้าม generic",
      "earn": "฿XX-XX ภายใน X วัน หรือ null ถ้ายังไม่เห็นเงินในขั้นนี้"
    },
    {
      "day": "วันที่ 3",
      "time": "XX นาที",
